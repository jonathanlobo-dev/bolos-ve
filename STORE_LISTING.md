# Ficha de Play Store — Bolos VE

## Título (máx. 30 caracteres)
Bolos VE: Dólar BCV y P2P

## Descripción corta (máx. 80 caracteres)
Dólar BCV, Euro y P2P en tiempo real, con calculadoras y alertas. Rápido y simple.

## Descripción completa (máx. 4000 caracteres)
Bolos VE es la forma más rápida y sencilla de ver el precio del dólar en Venezuela y hacer tus cálculos del día a día.

💱 TASAS AL INSTANTE
• Dólar BCV oficial, Euro BCV y P2P (USDT) actualizados.
• Mira el % de cambio y el valor anterior de cada tasa.
• Escribe un monto y velo convertido en todas las tasas a la vez.

🧮 CALCULADORAS ÚTILES
• Pago Móvil: calcula cuánto necesitas tener o cuánto puedes enviar contando la comisión del banco.
• ¿Me conviene?: te dice si te sale más barato pagar en dólares o en bolívares.

🔔 ALERTAS Y RECORDATORIOS
• Crea alertas de precio: te avisamos cuando el dólar cruza el valor que quieras.
• Notificación diaria con las tasas, a la hora que elijas.

⚙️ A TU GUSTO
• Elige qué tasas ver, en qué orden y en qué diseño (columnas o lista).
• Tema claro y oscuro.
• Mantén presionada una tarjeta para copiar el monto.
• Comparte las tasas del día por WhatsApp o Telegram.

Bolos VE funciona también sin conexión mostrando las últimas tasas guardadas.

Aviso: las tasas se muestran con fines informativos, obtenidas de fuentes públicas. No nos responsabilizamos por decisiones financieras basadas en esta información.

---

## Datos para el formulario de Play Console
- **Categoría:** Finanzas
- **Tipo:** App (gratuita)
- **Clasificación de contenido:** Para todos (responder cuestionario; no tiene contenido sensible)
- **Anuncios:** Sí (contiene anuncios — AdMob)
- **Seguridad de datos:** No se recopilan datos personales; los ajustes se guardan solo en el dispositivo. (AdMob puede recopilar identificadores para anuncios — declararlo en la sección de anuncios.)
- **Política de privacidad:** (la URL que obtengas al hostear privacy.html)

## Textos para redes / compartir (opcional)
"📲 Descarga Bolos VE: el dólar BCV, Euro y P2P en tiempo real, con calculadora de pago móvil y alertas. ¡Gratis!"
